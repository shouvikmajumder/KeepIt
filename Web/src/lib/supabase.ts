...
export const configurationError = !url || !key
  ? 'Set the public Supabase URL and key in Web/.env, then restart the web server.' : null;
...
export function exchangeCode(code: string) {
  if (exchange?.code !== code) {
    exchange = { code, result: supabase.auth.exchangeCodeForSession(code)
      .then(({ error }) => error
        ? 'This link expired or was opened in a different browser. Request a new link and open it in the browser where you requested it.' : null)
      .catch(() => 'Unable to verify this link. Check your connection and request a new link.') };
  }
