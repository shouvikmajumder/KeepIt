...
class ReviewTests(DatabaseCase):
    def test_concurrent_confirmation_creates_one_subscription(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            results = list(executor.map(lambda _: review(candidate, Review(action="add"), self.owner), range(4)))
    ...
    def test_review_cannot_access_another_user(self):
        ...
        with self.assertRaises(HTTPException) as error:
            review(candidate, Review(action="add"), self.other)
    ...
    def test_missing_date_and_unsupported_candidate(self):
        ...
        with self.assertRaises(HTTPException):
            review(candidate, Review(action="add"), self.owner)
        review(candidate, Review(action="add", next_renewal_date="2026-10-01"), self.owner)
        unsupported = self.candidate(connection, eligible=False, reason="Only USD")
        with self.assertRaises(HTTPException):
            review(unsupported, Review(action="add"), self.owner)
    ...
    def test_provider_updates_preserve_edits_and_ignored_decisions(self):
        ...
        result = review(candidate, Review(action="add"), self.owner)
    ...
    def test_disconnect_preserves_confirmed_subscriptions(self):
        connection = self.connection()
        result = review(self.candidate(connection), Review(action="add"), self.owner)
