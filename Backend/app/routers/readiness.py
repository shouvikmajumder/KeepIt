...
def ready():
    try:
        with transaction() as db:
            ...
            db.execute("select source,detection_key,account_id,last_seen_at from keepit_private.candidates limit 0")
            db.execute("select candidate_id,transaction_id from keepit_private.candidate_transactions limit 0")
    except Exception:
